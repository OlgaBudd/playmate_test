...
var Module = {
	...
	UE4_indexedDBName: 'UE4_assetDatabase_PlayMaxProject', // this should be an ascii ID string without special characters that is unique to the project that is being packaged
	UE4_indexedDBVersion: 202507311237, // Bump this number to invalidate existing IDB storages in browsers.
};
...
Module.arguments = ['../../../PlayMaxProject/PlayMaxProject.uproject','-stdout',];
...
//	}
	...
	function withIndexedDB(db) {
		...
		var mainCompiledCode = fetchFromIndexedDB(db, 'wasmModule').then(function(wasmModule) {
			...
			return download(Module.locateFile('https://dl.dropboxusercontent.com/scl/fi/l4aa7617wkwtwzjqhs3c8/PlayMaxProject-HTML5-Shipping.wasm?rlkey=h94k2hdjwg0f8r5cxwcr83dtp&st=ipiwthp6&dl=0'), 'arraybuffer').then(function(wasmBytes) {
		...
		// ----------------------------------------
		// MAIN JS
		var mainJsDownload = fetchOrDownloadAndStore(db, Module.locateFile('https://rawcdn.githack.com/OlgaBudd/playmate_test/refs/heads/main/PlayMaxProject-HTML5-Shipping.js?token=GHSAT0AAAAAADIL26XNCLI3P27OOZJJFPJC2ELP75Q'), 'blob').then(function(data) {
		...
		// ----------------------------------------
		// MORE JS
		var dataJsDownload = fetchOrDownloadAndStore(db, Module.locateFile('https://rawcdn.githack.com/OlgaBudd/playmate_test/refs/heads/main/PlayMaxProject-HTML5-Shipping.data.js?token=GHSAT0AAAAAADIL26XMIXVXQL3YL3CGGHVG2EMPPUQ'));
		var utilityJsDownload = fetchOrDownloadAndStore(db, Module.locateFile('https://rawcdn.githack.com/OlgaBudd/playmate_test/refs/heads/main/Utility.js?token=GHSAT0AAAAAADIL26XM46NEGTQLWPZQMEQY2EMPYFQ')).then(addScriptToDom);
		var dataDownload =
/* // The following code would download and store the .data file as a Blob, which should be more efficient than loading an ArrayBuffer. However that seems to be buggy, so avoid it for now.
			fetchOrDownloadAndStore(db, Module.locateFile('PlayMaxProject-HTML5-Shipping.data')).then(function(dataBlob) {
				return readBlobToArrayBuffer(dataBlob).then(function(dataArrayBuffer) {
					Module['preloadedPackages'] = {};
					Module['preloadedPackages'][Module.locateFile('PlayMaxProject-HTML5-Shipping.data')] = dataArrayBuffer;
...
// Instead as a fallback, download as ArrayBuffer. (TODO: Figure out the bugs with the above, and switch to using that one instead)
			fetchOrDownloadAndStore(db, Module.locateFile('https://dl.dropboxusercontent.com/scl/fi/u6yh8zaaik3ixeed67rb1/PlayMaxProject-HTML5-Shipping.data?rlkey=2yk03td0uwoi3tc1uoxhnqmrc&st=t9ng486u&dl=0'), 'arraybuffer').then(function(dataArrayBuffer) {
				Module['preloadedPackages'] = {};
				Module['preloadedPackages'][Module.locateFile('https://dl.dropboxusercontent.com/scl/fi/u6yh8zaaik3ixeed67rb1/PlayMaxProject-HTML5-Shipping.data?rlkey=2yk03td0uwoi3tc1uoxhnqmrc&st=t9ng486u&dl=0')] = dataArrayBuffer;
				return dataJsDownload.then(addScriptToDom);
			});
